...
public class Solution {
    //Good morning!
	static boolean valid( int r , int c ,int x , int y ,int v[][]){
		if(x<0 ||x>=r || y<0 || y>=c || v[x][y]==1){
			return false;
		}
		return true;
	}
	static int sol(int a[][] , int r , int c ,int mx, int x , int y ,int v[][]){
		if(!valid(r , c , x , y , v)){
			return 0;
		}
		v[x][y]=1;
		int sum=0;
		int max =0;
		if(Math.abs(mx - a[x][y])>1){
			sum+=Math.abs(mx - a[x][y])-1;
			a[x][y]=mx-1;
		}
		if(x-1>=0){
			max = Math.max(max , a[x-1][y]);
		}
		if(y-1>=0){
			max = Math.max(max , a[x][y-1]);
		}
		if(x+1<r){
			max = Math.max(max , a[x+1][y]);
		}
		...
		int z = a[x][y];
		sum+=sol(a , r , c ,z ,   x+1 , y,v);
		sum+=sol(a , r , c , z,x , y+1 , v );
		sum+=sol(a , r , c , z , x , y-1 , v);
		sum+=sol(a , r , c , z , x-1 , y , v);
		v[x][y]=0;
		return sum;
